...
/* ----------------------------------------------------------------------
   Contributing John Mitchell (Sandia) and Jeremy Trageser
------------------------------------------------------------------------- */
...
//     : PottsAmPathParser(spk, narg, arg), pool_width(0), pool_depth(0),
//     haz(0),
//       xt{0, 0, 0, 0, 0}, yt{0, 0, 0, 0, 0}, zs{0, 0, 0, 0, 0}, beta_y(1.0),
//       beta_z(0.5), distance(nullptr), bezier(), HAZ(), d_table() {
AppPottsAmBezier::AppPottsAmBezier(SPPARKS *spk, int narg, char **arg)
    : PottsAmPathParser(spk, narg, arg), pool_width(0), pool_depth(0), h0(0),
      ht(0), xt{0, 0, 0, 0, 0}, yt{0, 0, 0, 0, 0}, zs{0, 0, 0, 0, 0},
      beta_y(1.0), beta_z(0.5), distance(nullptr), HAZ(), d_table() {

  // only error check for this class, not derived classes
  if (strcmp(arg[0], "potts/am/bezier") != 0 || narg != 6)
...
void AppPottsAmBezier::init_app() {
  ...
  size_t nx = static_cast<size_t>(std::ceil(length + h0 + ht) + 1);
  // size_t ny = static_cast<size_t>(std::ceil(pool_width / 2 + haz) + 1);
  size_t ny = static_cast<size_t>(std::ceil(pool_width / 2 + h0) + 1);
  ...
  // Initialize table of distances to h0
  d_table.resize(N, h0);
