...
void Set::command(int narg, char **arg) {
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "fraction") == 0) {
      ...
      if (iarg + 4 > narg)
        error->all(FLERR, "Illegal set command");
      cond = (Condition *)memory->srealloc(
          cond, (ncondition + 1) * sizeof(Condition), "set:cond");
  ...
  if (domain->me == 0) {
    if (screen)
      fprintf(screen, "  " TAGINT_FORMAT " settings made for %s\n", allcount,
              arg[0]);
    if (logfile)
      fprintf(logfile, "  " TAGINT_FORMAT " settings made for %s\n", allcount,
              arg[0]);
  }
  ...
  if (loopflag) {
    ...
    if (lhs == SITE || lhs == IARRAY) {
      if (regionflag == 0 && fraction == 1.0) {
        ...
        for (i = 0; i < nlocal; i++)
          if (domain->regions[iregion]->match(xyz[i][0], xyz[i][1],
                                              xyz[i][2])) {
            if (ncondition && condition(i))
              continue;
            if (rhs == VALUE)
...
void Set::set_stitch(int lhs, int rhs) {
  ...
  // const char *fmt = "xlo,xhi,ylo,yhi,zlo,zhi:
  // %10.4f,%4d,%4d,%4d,%4d,%4d,%4d\n"; printf(fmt, time, xlo, xhi, ylo, yhi,
  // zlo, zhi);
...
#ifdef LOG_STITCH
  {
    ...
    fname << "read.blocks.np" << std::setw(num_digits) << std::setfill('0')
          << num_procs << ".";
    ...
    if (!fp) {
      error->all(FLERR,
                 "set.cpp; in function 'set_stitch' file opening failed.");
    }
  ...
  bool init = false;
  {
    ...
    if (STITCH_FIELD_NOT_FOUND_ID == field_id) {
      ...
      if (SITE == lhs || IARRAY == lhs) {
        // Specify value type of value and its no value present 'value'
        type = STITCH_INT32;
        union StitchTypesUnion nvp;
        nvp.i32 = -1;
        int32_t flen = 1;
        ...
        error->all(FLERR, "'set stitch' command failed on read; "
                          "Specified field does not exist; also "
                          "unable to "
                          "create field type specified.");
      }
      // field was initialized on 'stitch' file; set
      // 'init'=true
      init = true;
      int my_rank;
      MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
      if (0 == my_rank) {
        printf("'set stitch' creating field '%s' on "
               "'stitch file'\n",
               label);
      }
    }
...
void Set::set_binary_file(int lhs, int rhs) {
  ...
  if (me == 0) {
    ...
    if (fp == NULL)
      error->one(FLERR, "Set could not open binary file");
  ...
  if (lhs == SITE || lhs == IARRAY) {
    ...
    if (me == 0)
      tmp = fread(buf, sizeof(int), nx * ny * nz, fp);
    ...
    if (me == 0)
      tmp = fread(buf, sizeof(double), nx * ny * nz, fp);
    MPI_Bcast(buf, nx * ny * nz, MPI_INT, 0, world);
